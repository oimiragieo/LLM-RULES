...
Agent Studio is a pnpm-managed Node.js repository for agent, skill, hook, and tool configuration across Claude Code, Cursor, and Factory. **Start with [`claude-index.md`](claude-index.md)** for a folder-by-folder map (links into `docs/code-maps/*/MAP.md` with per-file purposes and extracted functions). Core implementation lives under `.claude/`: `.claude/lib/` for reusable runtime modules, `.claude/hooks/` for lifecycle hooks, `.claude/tools/` for CLI utilities, `.claude/agents/` for agent definitions, `.claude/skills/` for skill packages, and `.claude/templates/` or `.claude/schemas/` for contracts. Repository automation lives in `scripts/`. Tests live in `tests/`, usually mirroring the feature area, with shared fixtures in `tests/fixtures/`. Keep temporary or generated runtime data in `.tmp/`, `tmp/`, `local_cache/`, or `.claude/context/` unless a script expects tracked outputs. A large module reorg (flat→layered restructure, cross-cutting relayer) should follow the slice-loop rather than a single big-bang PR: measure fan-in/fan-out + import cycles BEFORE choosing layers, adversarially review the layer design, then land one independently-shippable slice at a time with CI as the merge gate. Skills: `refactor-flat-package-to-layers` + `use-cursor`.
