...
function generateReport(sessionId, transcriptEvents, debugErrors) {
    ...
    for (const event of transcriptEvents) {
        if (event.type === 'user' || event.type === 'assistant') {
            if (event.type === 'user' && Array.isArray(event.message?.content) && event.message.content.length > 0 && event.message.content[0].type === 'text') {
                userMessages++;
            }
    ...
    const routerLocks = debugErrors.filter(e => e.includes('ROUTER-LOCKDOWN') || e.includes('TASK-CREATE VIOLATION'));
    if (routerLocks.length > 0) {
        cleanSystem = false;
        lines.push('### ⚠️ Router Protocol Violations Detected');
        lines.push('The agent system locked down due to direct file modifications bypassing the router.');
...
function main() {
    ...
    const finalOutput = outputPath ? path.resolve(outputPath) : path.join(tmpDir, `transcript-analysis-${exactSessionId.slice(0, 8)}.md`);
