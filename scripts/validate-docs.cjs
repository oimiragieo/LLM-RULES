...
const linkRegex = /(?:\[[^\]]*\]\()?(?:[`'"]?)((?:@?\.claude\/|\.\/|\.\.\/|scripts\/|tests\/)[a-zA-Z0-9_\-./]+(?:#.*)?)(?:[`'"]?)?/g;
    ...
    lines.forEach((lineText, lineIdx) => {
        const checkLine = (regex) => {
            ...
            while ((localMatch = regex.exec(lineText)) !== null) {
                ...
                if (!cleanLink || cleanLink.startsWith('http') || cleanLink.includes('{{') || cleanLink.includes('<') || cleanLink.startsWith('mailto:')) continue;
                if (cleanLink.match(/^\*(.*)\*$/)) continue;
                if (cleanLink === '.' || cleanLink === '..') continue;
                if (cleanLink.includes('()')) continue;

                let absolutePath = '';
                if (cleanLink.startsWith('.claude/') || cleanLink.startsWith('scripts/') || cleanLink.startsWith('tests/')) {
                    absolutePath = path.join(ROOT, cleanLink);
                } else if (cleanLink.startsWith('./') || cleanLink.startsWith('../') || !cleanLink.startsWith('/')) {
                ...
                if (absolutePath && !fs.existsSync(absolutePath)) {
                    ...
                    if (absolutePath.includes('example') || absolutePath.includes('my-') || absolutePath.includes('/...')) continue;
                    errors.push({ file: relFile, lineNum: lineIdx + 1, originalLink: linkPath, resolvedPath: path.relative(ROOT, absolutePath) });
                }
            }
        };
        checkLine(linkRegex);
...
fs.writeFileSync(path.join(ROOT, '.claude', 'context', 'tmp', 'docs-broken-links.json'), JSON.stringify(uniqueErrors, null, 2));
console.log(`Saved ${uniqueErrors.length} unique broken links to .claude/context/tmp/docs-broken-links.json`);
