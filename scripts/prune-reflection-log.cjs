...
if (!fs.existsSync(LOG_PATH)) { console.log('OK: reflection-log.jsonl not found'); process.exit(0); }
const stat = fs.statSync(LOG_PATH);
if (stat.size <= MAX_BYTES) { console.log(`OK: reflection-log.jsonl is ${(stat.size/1024).toFixed(1)}KB, no pruning needed`); process.exit(0); }
...
console.log(`Pruned: ${archive.length} lines archived to ${path.basename(archivePath)}, ${keep.length} lines kept (was ${(stat.size/1024).toFixed(1)}KB)`);
