...
function checkRequires(filePath) {
    ...
    while ((match = requireRegex.exec(content)) !== null) {
        ...
        if (reqPath.startsWith('.')) {
            ...
            if (!found) {
                errors.push(`[PHANTOM_REQUIRE] Broken require mapping in ${path.relative(ROOT, filePath)}: ${reqPath}`);
            }
        }
    }
}
    ...
    for (const agentFile of agents) {
        ...
        if (skillsMatch) {
            ...
            if (skillsLines.length > 50) {
                warnings.push(`[AGENT_BLOAT] ${relFile} has highly bloated skills list (${skillsLines.length} skills)`);
            }
    ...
    if (fs.existsSync(catalogPath)) {
        try {
            ...
            if (catalogCount !== agentCount) {
                warnings.push(`[STALE_CATALOG] agent-registry.json has ${catalogCount} entries, but found ${agentCount} agent files.`);
            }
...
const allScriptFiles = walk(ROOT).filter(f => f.endsWith('.js') || f.endsWith('.cjs') || f.endsWith('.mjs'));
