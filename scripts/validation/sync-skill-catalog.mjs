...
const CATALOG_PATH = path.join(PROJECT_ROOT, '.claude', 'context', 'artifacts', 'catalogs', 'skill-catalog.md');
...
function main() {
    ...
    for (let i = 0; i < catalogLines.length; i++) {
        ...
        if (match) {
            ...
            if (/all agents/i.test(lastCell) || /\d+\+\s*agents/i.test(lastCell) || /all creators/i.test(lastCell) || /\(all [^)]*\)/i.test(lastCell) || /^all\s/i.test(lastCell)) {
                continue;
            }
            ...
            const currentNorm = currentAgentsStr.split(',').map(s => s.trim()).filter(Boolean).sort().join(',');
            const newNorm = primaryAgents.map(s => s.trim()).filter(Boolean).sort().join(',');
