...
- **Code-indexing uses LanceDB**: Code indexing now uses LanceDB (table `code_index` by default) via `vector-store.cjs` and `index-manager.cjs`. The persist directory is `.claude/data/lancedb` (override via `LANCEDB_URI`), and the table name is configurable via `LANCEDB_TABLE_CODE`. MEMORY_SYSTEM.md documents the new code-indexing storage and `pnpm run code:index:reindex`.
