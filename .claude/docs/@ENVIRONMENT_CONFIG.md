...
| `RAG_AT_SPAWN_LIMIT`                      | number     | 5          | Max matches fetched for `assembleSpawnPromptAsync(memoryQuery)` retrieval.                                            |
| `RAG_AT_SPAWN_THRESHOLD`                  | number     | (unset)    | Optional threshold for async RAG retrieval at spawn.                                                                  |
| `RAG_AT_SPAWN_MAX_ITEMS`                  | number     | 5          | Max bullets rendered in `Task-Relevant Memory (RAG)` subsection.                                                      |
