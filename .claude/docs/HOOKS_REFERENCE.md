...
- `aip-token-injector.cjs`: PreToolUse(Task) hook that auto-injects an AIP Invocation-Bound Capability Token (arXiv 2603.24775) into every Task() spawn. Resolves subagent capabilities from the manifest capability map and signs a token with the AIP signing key. Escape hatch: `AIP_TOKENS=off`. Fail-open (advisory — does not block spawns on failure).
- `aip-token-validator.cjs`: PreToolUse(Task) hook that validates the capability token injected by `aip-token-injector.cjs`. Verifies signature, expiry, delegatee, and a representative capability before allowing the spawn. In production mode (`AIP_TOKENS` != `off`): fail-closed for invalid tokens. Escape hatch: `AIP_TOKENS=off` passes through.
