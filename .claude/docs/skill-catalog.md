...
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tdd`                            | Canon TDD with scenario backlog and RED/GREEN proof.                                                                                                                                                                                                                       | developer, qa, python-pro, rust-pro, golang-pro, typescript-pro, fastapi-pro, nodejs-pro, ios-pro, java-pro, php-pro, sveltekit-expert, tauri-desktop-developer, expo-mobile-developer, data-engineer, graphql-pro, code-reviewer |
| `debug-log-analysis`             | Structured debug log analysis for Claude Code sessions — copy log, run reducer, extract error patterns, correlate with full log, produce observability report. Fills 5 identified gaps: hook error body, agent identity, file path, stall correlation, success visibility. | reflection-agent, devops-troubleshooter, developer                                                                                                                                                                                |
...
| `session-transcript-analyzer`    | Parses and merges Claude .jsonl transcripts with debug logs to generate a timeline heuristics report of API limits, context overflows, hook blocks, and tool failures.                                                                                                     | reflection-agent, devops-troubleshooter, developer, qa                                                                                                                                                                            |
...
| `ralph-loop`            | Autonomous iteration loop using Claude Code Stop hooks — verification-first exit gates, circuit breaker, state persistence, guardrails                                 | developer, qa, devops, master-orchestrator                 |
...
| `drizzle-orm-rules` | Rules for using Drizzle ORM within the src/lib/db directory. Ensures consistent data modeling and database interactions. | developer, database-architect |
...
| `authentication-flow-rules`    | OAuth 2.1 compliant authentication flows (PKCE required for all clients, Implicit Flow removed, modern token security)          | security-architect, developer        |
...
| `building-secure-contracts`    | Smart contract and secure API contract security: invariant checking, access control, reentrancy, integer overflow patterns      | security-architect, developer        |
| `yara-authoring`               | YARA-X detection rule authoring for malware detection, threat hunting, and IOC identification with atom analysis and linting    | reverse-engineer, security-architect |
...
| `feature-flag-management`     | Feature flag lifecycle -- safe toggling, gradual rollouts, A/B testing, flag cleanup | developer, devops, qa         |
...
| `modern-python`                    | Modern Python tooling with uv, ruff, ty, and pytest (Trail of Bits standards)    | python-pro, developer, fastapi-pro                                                |
| `poetry-rye-dependency-management` | Python dependency management with Poetry and Rye -- lockfiles, groups, migration | python-pro, developer                                                             |
| `pyqt6-ui-development-rules`       | PyQt6 desktop GUI development -- signal/slot architecture, QSS theming, QThread  | python-pro, developer                                                             |
| `powershell-expert`                | PowerShell 7+ scripting, system administration, Pester testing, DSC, JEA         | developer, devops                                                                 |
...
| `context-compressor` | Search-aware context compression with MemoryRecord mapping   | developer                                                                    |
...
| `agent-evaluation`                   | LLM-as-judge evaluation framework with 5-dimension rubric for scoring AI-generated content quality                                                                                          | qa, code-reviewer, reflection-agent |
...
| `property-based-testing`             | fast-check patterns for JS/TS — 6 canonical property categories with worked examples targeting agent-studio utilities                                                                       | qa, developer                       |
...
| `omega-claude-cli` | Headless wrapper for Claude Code CLI; spawns a second Claude session headlessly for cross-validation, second opinions, and isolated analysis without sharing current agent context | developer, multi-llm-consultant |
| `llm-council` | Orchestrate multi-LLM parallel debate and synthesis; dispatches prompts to omega CLI wrappers in parallel, collects responses, runs peer review, and synthesizes via chairman model | multi-llm-consultant, developer |
...
| Skill                                     | Description                                                          | Primary Agents             |
| ----------------------------------------- | -------------------------------------------------------------------- | -------------------------- |
| `scientific-skills/rdkit`                 | Cheminformatics with RDKit for molecular analysis and drug discovery | scientific-research-expert |
| `scientific-skills/scanpy`                | Single-cell RNA-seq analysis with Scanpy and AnnData                 | scientific-research-expert |
| `scientific-skills/biopython`             | Bioinformatics sequence analysis with BioPython and pysam            | scientific-research-expert |
| `scientific-skills/literature-review`     | PRISMA-compliant systematic literature review workflow               | scientific-research-expert |
| `scientific-skills/hypothesis-generation` | 8-step hypothesis generation and experimental design methodology     | scientific-research-expert |
| `scientific-skills/scientific-schematics` | Scientific schematic and diagram generation for publications         | scientific-research-expert |
...
| `fix-review`                                   | Verify fix commits address security findings without regressions                                                                                                       | security-architect, code-reviewer |
...
| `webapp-testing`                               | Test local web applications with Playwright and Python                                                                                                                 | qa, frontend-pro                  |
...
| ---------------- | ---------------- | ---------------------------------------------- |
| `writing`        | `writing-skills` | [DEPRECATED ALIAS] Redirects to writing-skills |
