...
| `tools`           | No       | Array of specific tools the subagent can use, for example `\["Read", "Edit", "Bash"]`. If omitted, inherits all tools. Supports \[`Task(agent\_type)`](/en/sub-agents#restrict-which-subagents-can-be-spawned) syntax |
