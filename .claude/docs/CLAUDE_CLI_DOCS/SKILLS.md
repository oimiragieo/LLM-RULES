...
&nbsp; CLAUDE.md files from `--add-dir` directories are not loaded by default. To load them, set `CLAUDE\_CODE\_ADDITIONAL\_DIRECTORIES\_CLAUDE\_MD=1`. See \[Load memory from additional directories](/en/memory#load-memory-from-additional-directories).
...
To override the limit, set the `SLASH\_COMMAND\_TOOL\_CHAR\_BUDGET` environment variable.
