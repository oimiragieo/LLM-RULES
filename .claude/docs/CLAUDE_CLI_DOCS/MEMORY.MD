...
&nbsp; Auto memory is being rolled out gradually. If you aren't seeing auto memory, you can opt in by setting `CLAUDE\_CODE\_DISABLE\_AUTO\_MEMORY=0` in your environment.
...
export CLAUDE\_CODE\_DISABLE\_AUTO\_MEMORY=1  # Force off

export CLAUDE\_CODE\_DISABLE\_AUTO\_MEMORY=0  # Force on
...
To also load memory files (CLAUDE.md, .claude/CLAUDE.md, and .claude/rules/\\\*.md) from additional directories, set the `CLAUDE\_CODE\_ADDITIONAL\_DIRECTORIES\_CLAUDE\_MD` environment variable:
