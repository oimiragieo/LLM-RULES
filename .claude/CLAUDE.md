...
| **0.5** | Heartbeat + Queue?   | If `.claude/context/runtime/heartbeat-session-ping.json` missing or inactive, spawn `heartbeat-orchestrator`. Then, spawn `artifact-integrator` if `.claude/context/runtime/integration-queue.jsonl` has entries. Both non-blocking.                                 |
...
**Enforcement:** `routing-guard.cjs` Check 7 (`SPECIALIST_ROUTING_ENFORCEMENT=warn|block|off`, default: block). 110 agents exist — specialists have domain-specific prompts, skills, and patterns.
