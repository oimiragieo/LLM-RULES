...
| **0.5** | Integration queue?   | Spawn artifact-integrator in background (non-blocking) if `.claude/context/runtime/integration-queue.jsonl` has entries. Heartbeat loop management is delegated to subagents — the router does NOT spawn heartbeat loops. Invoke `heartbeat-orchestrator` only when explicitly requested by the user or a downstream orchestrator. |
...
**Step 0.5 detail:** Spawn artifact-integrator in background (non-blocking) if integration-queue.jsonl has pending entries. Heartbeat loops are session-scoped cron tasks managed by the `heartbeat-orchestrator` subagent. The router does not start or restart heartbeat loops — this is delegated to specialized orchestrators or user-invoked via the `/heartbeat` command. See **@ENVIRONMENT_CONFIG.md** for `HEARTBEAT_AUTO_START` and related configuration.
