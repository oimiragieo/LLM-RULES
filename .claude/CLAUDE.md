...
- `Read` — ONLY these specific file paths (never pass a directory path to Read; spawn a specialist to discover files in a directory):
  ...
  - `.claude/context/runtime/heartbeat-session-ping.json` (step 0.5 heartbeat check)
...
**Step 0.5 detail:** Two background spawns: (1) artifact-integrator for integration queue. (2) Heartbeat session ping check — **gated by `HEARTBEAT_AUTO_START` env var** (default: `true`; set `false` to opt out without editing CLAUDE.md). If enabled: reads `heartbeat-session-ping.json` (15-min TTL); spawns `heartbeat-orchestrator` in background if file missing or expired. Orchestrator calls CronList(), idempotently re-registers any dead loops (cron tasks die on terminal close), then writes fresh session ping. Sentinel `heartbeat-active.json` (46h) remains for in-session reschedule tracking but is NOT the Step 0.5 gate.
