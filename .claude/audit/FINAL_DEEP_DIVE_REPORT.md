...
*   **Worker Agent**: `worker-agent.cjs` is *not* a stub. It is a fully functional maintenance loop that respects `WORKER_ENABLED` env var. It is currently the only mechanism to drive `memory-scheduler.cjs` automatically.
