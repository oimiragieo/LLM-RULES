...
```json
{
  "matcher": "Task",
  "hooks": [
    { "type": "command", "command": "node .claude/hooks/routing/agent-health-hook.cjs" }
  ]
}
```
...
```javascript
const ROUTER_ENFORCER = path.join(PROJECT_ROOT, '.claude', 'hooks', 'lib', 'routing', 'routing-table.cjs');
```
...
*Report generated by Kiro AI - February 4, 2026*
*Revision 2: Deep Dive Update with additional critical findings*
