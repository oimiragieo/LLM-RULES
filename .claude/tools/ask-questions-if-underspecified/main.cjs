...
function main() {
  ...
  const skillPath = path.resolve(__dirname, '../../skills/ask-questions-if-underspecified/SKILL.md');
  ...
  const result = {
    ...
    status: 'ready',
    skillPath: skillPath,
    message: 'Skill ask-questions-if-underspecified is available. Invoke via Skill({ skill: "ask-questions-if-underspecified" }) in agent context.'
  };
