...
export async function validateAgentInJsRouting(agentName) {
    try {
        ...
        const { INTENT_TO_AGENT, ROUTING_TABLE } = await import('../../lib/routing/routing-table-data.cjs');
        ...
        if (found) {
            return { passed: true, message: `Found '${agentName}' in JS routing exports via AST/Logical check` };
        }
        return { passed: false, message: `Could not find logical routing target '${agentName}' in router mappings` };
    } catch (err) {
 ...
 * Uses remark-parse.
 */
export async function validateAgentInMarkdownTables(agentName, mdFilePath = path.join(PROJECT_ROOT, '.claude', 'CLAUDE.md')) {
