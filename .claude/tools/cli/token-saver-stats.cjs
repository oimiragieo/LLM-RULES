...
const TELEMETRY_FILE = path.join(PROJECT_ROOT, '.claude', 'context', 'runtime', 'token-saver-context-compression', 'token-saver-telemetry.jsonl');
...
function main() {
    ...
    if (!fs.existsSync(TELEMETRY_FILE)) {
        ...
        console.log('Use `Skill({ skill: "token-saver-context-compression" })` to start saving tokens.');
    ...
    if (uniqueModels.length === 0 || (uniqueModels.length === 1 && uniqueModels[0].includes('sonnet'))) {
    ...
    console.log(`\x1b[1mTotal Tokens Searched\x1b[0m  : \x1b[36m${formatNumber(totalOriginal)}\x1b[0m tokens`);
    console.log(`\x1b[1mTotal Tokens Output\x1b[0m    : \x1b[36m${formatNumber(totalCompressed)}\x1b[0m tokens`);
    console.log(`\x1b[1mAverage Reduction\x1b[0m      : \x1b[35m${avgCompressionRatio.toFixed(1)}%\x1b[0m`);
    console.log('');
    console.log('\x1b[36m-------------------------------------------------\x1b[0m');
    console.log(`\x1b[1m\x1b[32mTOTAL TOKENS SAVED\x1b[0m     : \x1b[1m\x1b[32m${formatNumber(totalSaved)}\x1b[0m tokens`);
    console.log(`\x1b[1m\x1b[32mTOTAL MONEY SAVED\x1b[0m      : \x1b[1m\x1b[32m$${totalUsd.toFixed(4)}\x1b[0m \x1b[90m${costLabel}\x1b[0m`);
    console.log('\x1b[36m-------------------------------------------------\x1b[0m');
    ...
    if (options.detailed && logs.length > 0) {
        ...
        for (const log of recent) {
            ...
            console.log(`\x1b[90m[${date}]\x1b[0m "${log.query.substring(0, 30)}${log.query.length > 30 ? '...' : ''}" \x1b[32mSaved ${saved} tokens (${money})\x1b[0m`);
        }
    }
}
