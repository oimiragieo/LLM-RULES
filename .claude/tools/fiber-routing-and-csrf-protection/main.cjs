...
function main() {
  ...
  const skillPath = path.resolve(__dirname, '../../skills/fiber-routing-and-csrf-protection/SKILL.md');
  ...
  const result = {
    ...
    status: 'ready',
    skillPath: skillPath,
    message: 'Skill fiber-routing-and-csrf-protection is available. Invoke via Skill({ skill: "fiber-routing-and-csrf-protection" }) in agent context.'
  };
