...
When the remediation is run with `{ dryRun: true }`, the returned plan lists all files that *would* be created/modified, but no files are actually written. A hash of the working tree before and after is identical.
