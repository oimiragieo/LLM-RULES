...
const MEMORY_SYNC_STATE_FILE = process.env.MEMORY_SYNC_STATE_FILE ||
...
function checkDynamicMemorySync(hookInput) {
    const sessionId = hookInput?.session_id || hookInput?.sessionId || process.env.CLAUDE_SESSION_ID || 'unknown';
    ...
    const sessionState = state.sessions[sessionId] || { lastNotifiedMtime: null, firstSeenMtime: latestMtime };
    ...
    if (hasUpdate) {
        ...
        if (keys.length > 200) {
            keys
                .sort((a, b) => Number(state.sessions[b].lastNotifiedMtime || 0) - Number(state.sessions[a].lastNotifiedMtime || 0))
                .slice(200)
        ...
        return {
            hasUpdate: true,
            warning: 'System global memory was updated dynamically by another agent since your session started. If you are stuck or encountering errors, run the `memory-search` skill: `node .claude/lib/memory/memory-search.cjs "<query>"` to fetch latest contexts.'
        };
    }
