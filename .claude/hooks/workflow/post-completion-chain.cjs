...
const {
  triggerMemoryExtraction: _triggerMemoryExtraction,
  MEMORY_EXTRACTION_TIMEOUT_MS,
  MEMORY_CONFIDENCE_THRESHOLD,
} = require('../../lib/memory/completion-memory-writer.cjs');
 ...
 * Delegates to completion-memory-writer.cjs which owns the withFileLock domain.
 * Separated to enforce LOCK_ORDER: withWorkflowStateLock and withFileLock must not
...
function triggerMemoryExtraction(metadata, taskId) {
  return _triggerMemoryExtraction(metadata, taskId);
}
