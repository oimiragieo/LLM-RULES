...
/**
 * template-creator - Post-Execute Hook
 * ==================================
 *
 * Runs after the template-creator executes to clean up state.
 *
...
function processResult(executionResult) {
  ...
  if (executionResult.success) {
    console.log(`[${CREATOR_NAME.toUpperCase()}] Template created successfully: ${executionResult.artifactName || 'unknown'}`);
  } else {
    console.warn(`[${CREATOR_NAME.toUpperCase()}] Template creation failed: ${executionResult.error || 'unknown error'}`);
  }
