...
/**
 * skill-creator - Post-Execute Hook
 * ==================================
 *
 * Runs after the skill-creator executes to clean up state.
 *
...
function processResult(executionResult) {
  ...
  if (executionResult.success) {
    console.log(
      `[${CREATOR_NAME.toUpperCase()}] Skill created successfully: ${executionResult.artifactName || 'unknown'}`
    );
  } else {
    console.warn(
      `[${CREATOR_NAME.toUpperCase()}] Skill creation failed: ${executionResult.error || 'unknown error'}`
    );
  }
