...
/**
 * schema-creator - Pre-Execute Hook
 * Runs before the skill executes to validate input and mark schema-creator as active.
 *
 * CRITICAL: This hook creates the state file that unified-creator-guard.cjs checks.
 * Without this, the guard has no way to know schema-creator was invoked.
 *
 * State file: .claude/context/runtime/active-creators.json
 * Format: { "schema-creator": { "active": true, "invokedAt": "ISO string", "ttl": 600000 } }
 */
...
const CREATOR_NAME = 'schema-creator';
...
/**
 * Mark schema-creator as active by updating unified state file.
 * This allows unified-creator-guard.cjs to know that schema writes are legitimate.
 */
...
function validateInput(_input) {
  ...
  // Basic validation - schema-creator can work with minimal input
  // The actual schema name is typically determined during the workflow

  return errors;
}

// Mark schema-creator as active FIRST (critical for unified-creator-guard)
const stateCreated = markCreatorActive();

if (!stateCreated) {
  console.warn(
    `[${CREATOR_NAME.toUpperCase()}] Warning: Could not create state file. unified-creator-guard may block schema writes.`
  );
}
...
console.log(
  `[${CREATOR_NAME.toUpperCase()}] Pre-execute complete. Schema-creator workflow is now active.`
);
process.exit(0);
