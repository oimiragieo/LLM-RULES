...
**In agent-studio:** When the `mcp__claude-flow__*` or `npx claude-flow` examples below are not available, use the Task tool to spawn agents (e.g. planner, developer, qa, architect) and follow the workflows in `.claude/workflows/` and router-decision.md instead.
