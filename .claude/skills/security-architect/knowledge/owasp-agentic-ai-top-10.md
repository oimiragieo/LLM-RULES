...
function filterSensitiveOutput(output) {
  let filtered = output;
  SENSITIVE_PATTERNS.forEach(p => { filtered = filtered.replace(p, '[REDACTED]'); });
  return filtered;
}
```
...
function validateSpawnDepth(currentDepth) {
  if (currentDepth >= MAX_SPAWN_DEPTH) throw new Error(`Max spawn depth ${MAX_SPAWN_DEPTH} exceeded`);
}
```
