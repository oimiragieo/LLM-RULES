...
function processResult(executionResult) {
  ...
  if (executionResult.success) {
    console.log(
      `[${CREATOR_NAME.toUpperCase()}] Hook created successfully: ${executionResult.artifactName || 'unknown'}`
    );
  } else {
    console.warn(
      `[${CREATOR_NAME.toUpperCase()}] Hook creation failed: ${executionResult.error || 'unknown error'}`
    );
  }
