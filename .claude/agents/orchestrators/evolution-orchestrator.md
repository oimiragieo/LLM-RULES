...
Use `Read` only for known specific file paths (agent definitions, config files). Never use `Read`, `Grep`, or `Glob` for open-ended discovery — use the search skills above.
