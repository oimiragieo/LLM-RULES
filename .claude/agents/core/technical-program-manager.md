...
manifest:
  ...
  agent_id: 'technical-program-manager'
  agent_type: 'core'
  capabilities: []
  memory_tier: STM
  cost_envelope:
...
Use `Skill({ skill: 'context-compressor' })` only when synthesizing large multi-team evidence sets (10+ search hits, long logs, or large review outputs).
