# Resolved Issues - February 2026

**Total Resolved This Month**: 22
**Archive Date**: 2026-02-04
**Source**: issues.md (main file now contains only OPEN issues)

---

## Index

1. DATA-INTEGRITY-001
2. RESOLVED
3. RESOLVED
4. SHELL-SECURITY-003
5. SHELL-SECURITY-004
6. RESOLVED
7. RESOLVED
8. META-001
9. META-002
...
## [DATA-INTEGRITY-001] Memory File Archival Missing Verification (HIGH) 🔴 RESOLVED

- **Date**: 2026-02-05
- **Severity**: HIGH
- **Status**: RESOLVED
- **Category**: data_integrity
- **Discovery**: Debug log analysis showed issues.md reduced from 2,505 to 307 lines
- **Description**: Agent performed "archival" of issues.md claiming to archive 23 resolved issues, but:
  1. Archive file `archive/issues-resolved-2026-02.md` was nearly empty (7 lines, header only)
  2. Source file issues.md was modified to reflect "successful" archival
  3. 2,018 lines of issue history were deleted but NOT copied to archive
  4. No verification step confirmed archive content before modifying source
- **Root Cause**:
  - Agent wrote archive header but failed to write issue content
  - Source file was modified regardless of archive completion
  - No "archive verification" step before declaring success
- **Impact**:
  - 87% data loss (2,198 of 2,505 lines)
  - Issue tracking history nearly destroyed
  - Audit trail incomplete
- **Resolution**:
  - Recovered via `git checkout HEAD~1 -- .claude/context/memory/issues.md`
  - Full 2,325 lines restored from git history
  - Added learning pattern for archival verification
- **Resolution Date**: 2026-02-05
- **Prevention**:
  1. Always verify archive content BEFORE modifying source
  2. Compare line counts: source_before = archive_after + source_after (approx)
  3. Read archive back and verify expected entries present
  4. Consider adding archival-verification hook
- **Related**: Pattern documented in learnings.md

---

## [RESOLVED] [SHELL-SECURITY-001] Background Bash Tasks Missing CWD Initialization [Date: 2026-02-04]
...
- **Resolution**: bash-cwd-validator.cjs registered and working (settings.json line 58)
...
## [RESOLVED] [SHELL-SECURITY-002] No Shell Injection Validation [Date: 2026-02-04]
...
- **Status**: RESOLVED
- **Category**: shell_security
- **Resolution**: shell-injection-validator.cjs registered and working (settings.json line 62)
- **Discovery**: Background Task Shell Command Audit (AUDIT-2026-01-31-SHELL-INJECTION)
...
## [RESOLVED] [CONFIG-001] Router Ignores config.yaml Agent Model Configuration (ADR-075) [Date: 2026-02-04]
...
- **Resolution**: config-model-validator.cjs implemented and registered (settings.json line 176), ADR-075 documents model resolution precedence
...
## [RESOLVED] [ROUTER-MONITORING-001] Router Stops Monitoring Subagents Mid-Project [Date: 2026-02-04]
...
**Resolution**: Fix applied (removed exitAgentMode call), hooks registered and working. Router now keeps agent mode active until subagent completes.
