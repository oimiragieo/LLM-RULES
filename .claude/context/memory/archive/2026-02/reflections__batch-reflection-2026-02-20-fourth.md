...
**Medium Priority:**
4. Add filesModified requirement to skill-updater task spawn prompts — current metadata gaps (IDs 2 and 4) make filesystem grep the only verification path
5. Standardize completion event deduplication: if two events fire within 60s for same task_id, suppress the second reflection spawn

**Low Priority:**
6. Align step numbering in skill-creator/skill-updater research sections (Step 2A label vs 1./2. numbering)
