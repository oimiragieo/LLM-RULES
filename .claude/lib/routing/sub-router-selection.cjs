...
const SUB_ROUTER_CONFIG = Object.freeze({
  'domain-router-web-frontend': Object.freeze({
    ...
    rules: Object.freeze([
      {
        agent: 'nextjs-pro',
        signals: ['next.js', 'nextjs', 'app router', 'server component', 'rsc', 'vercel'],
      },
  ...
  'domain-router-backend': Object.freeze({
    ...
    rules: Object.freeze([
      ...
      {
        agent: 'spring-boot-pro',
        signals: ['spring boot', '@springbootapplication', 'application.yml', 'spring mvc'],
      },
      {
        agent: 'nodejs-pro',
        signals: ['nestjs', 'express', 'node.js', 'nodejs', 'node ', 'package.json'],
      },
  ...
  'domain-router-mobile': Object.freeze({
    ...
    rules: Object.freeze([
      ...
      {
        agent: 'android-pro',
        signals: ['jetpack compose', 'android', 'gradle', 'adb', 'androidx'],
      },
      ...
      { agent: 'tauri-desktop-developer', signals: ['tauri', 'desktop app', 'desktop shell'] },
      {
        ...
        signals: ['mobile ux', 'usability', 'hig', 'mobile accessibility', 'design critique'],
      },
    ]),
  }),
  'domain-router-ai-ml': Object.freeze({
    ...
    rules: Object.freeze([
      ...
      },
      {
        ...
        signals: ['few-shot', 'few shot', 'system prompt', 'prompt optimization', 'prompt tuning'],
      },
      {
        agent: 'data-engineer',
        signals: ['etl', 'data warehouse', 'dbt', 'stream processing', 'feature pipeline'],
      },
      {
      ...
      {
        agent: 'ml-researcher',
        signals: ['research paper', 'ablation', 'novel architecture', 'experiment design'],
      },
      {
        agent: 'mlops-engineer',
        signals: ['mlflow', 'model registry', 'deploy model', 'serving', 'model monitoring'],
      },
      {
        agent: 'nlp-engineer',
        signals: ['named entity recognition', 'ner', 'tokenization', 'sentiment analysis', 'nlp'],
      },
      {
        agent: 'mcp-developer',
        signals: ['model context protocol', 'mcp server', 'mcp client', 'mcp'],
      },
      {
        agent: 'multi-llm-consultant',
        signals: ['compare llms', 'llm council', 'model comparison', 'choose the best model'],
      },
      {
        agent: 'model-benchmarker-agent',
        signals: ['benchmark', 'eval harness', 'evaluate model', 'model benchmark'],
      },
      {
    ...
    rules: Object.freeze([
      {
        ...
        signals: ['pagerduty', 'incident', 'outage', 'sev1', 'mitigation'],
      },
      {
        agent: 'sre-engineer',
        signals: ['slo', 'sli', 'error budget', 'reliability', 'capacity planning'],
      },
      {
        agent: 'kubernetes-specialist',
        signals: ['kubernetes', 'k8s', 'helm', 'argocd', 'kubectl', 'cluster'],
      },
      {
        agent: 'terraform-engineer',
        signals: ['terraform', 'iac module', '.tf', 'terraform plan'],
      },
      { agent: 'terragrunt-pro', signals: ['terragrunt'] },
      { agent: 'azure-infra-pro', signals: ['azure', 'aks', 'bicep'] },
      {
        agent: 'windows-infra-pro',
        signals: ['windows server', 'powershell remoting', 'active directory'],
      },
    ...
    rules: Object.freeze([
      {
        ...
        signals: ['xss', 'sql injection', 'penetration test', 'auth bypass', 'owasp'],
      },
      {
        agent: 'chaos-engineer',
        signals: ['chaos engineering', 'failure injection', 'game day', 'resilience test'],
      },
      {
        agent: 'reverse-engineer',
        signals: ['reverse engineer', 'decompile', 'binary analysis', 'firmware'],
      },
      {
        agent: 'advanced-debugging',
        signals: ['race condition', 'heap dump', 'segfault', 'thread dump', 'root cause'],
      },
      {
        agent: 'performance-engineer',
        signals: ['load test', 'profiling', 'latency', 'throughput', 'performance'],
      },
      {
        agent: 'accessibility-tester',
        signals: ['wcag', 'screen reader', 'keyboard navigation', 'color contrast', 'a11y'],
      },
      {
        agent: 'compliance-checker',
        signals: ['gdpr', 'soc 2', 'hipaa', 'regulatory', 'compliance'],
      },
      {
        agent: 'security-architect',
        signals: ['threat model', 'security architecture', 'secure design'],
      },
    ]),
  }),
  'domain-router-arch-data': Object.freeze({
    ...
    rules: Object.freeze([
      { agent: 'graphql-pro', signals: ['graphql', 'apollo', 'federation', 'schema stitching'] },
      {
        ...
        signals: ['microservices', 'service mesh', 'bounded context', 'event sourcing', 'cqrs'],
      },
      { agent: 'postgres-pro', signals: ['postgres', 'postgresql', 'vacuum', 'index bloat'] },
      {
        agent: 'sql-pro',
        signals: ['sql query', 'query plan', 'join optimization', 'window function'],
      },
      {
        agent: 'database-architect',
        signals: ['database schema', 'data model', 'er diagram', 'schema design'],
      },
  ...
  'domain-router-product': Object.freeze({
    ...
    rules: Object.freeze([
      {
        agent: 'ux-researcher',
        signals: ['user interview', 'usability study', 'research synthesis', 'persona'],
      },
      {
        agent: 'marketing-strategist',
        signals: ['growth campaign', 'marketing funnel', 'go to market', 'content strategy'],
      },
      { agent: 'brand-guardian', signals: ['brand voice', 'brand guideline', 'tone consistency'] },
      {
        agent: 'feedback-synthesizer',
        signals: ['feedback analysis', 'voice of customer', 'survey synthesis'],
      },
      {
        agent: 'legal-advisor',
        signals: ['contract review', 'terms of service', 'privacy policy', 'licensing'],
      },
      ...
      {
        agent: 'voice-replicator-agent',
        signals: ['voice replication', 'match voice', 'style transfer'],
      },
      {
        agent: 'forum-monitor-agent',
        signals: ['community moderation', 'forum monitoring', 'discord sentiment'],
      },
      {
        agent: 'post-analyzer-agent',
        signals: ['social post analysis', 'content performance', 'post engagement'],
      },
      {
      ...
      },
      {
        ...
        signals: ['stakeholder requirement', 'business case', 'process mapping'],
      },
      {
        agent: 'product-manager',
        signals: ['product strategy', 'roadmap strategy', 'positioning'],
      },
      { agent: 'pm', signals: ['prd', 'user story', 'acceptance criteria', 'backlog'] },
      {
        agent: 'pm-coordinator',
        signals: ['sprint planning', 'kanban', 'agile delivery', 'project coordination'],
      },
    ]),
  }),
  'domain-router-niche': Object.freeze({
    ...
    rules: Object.freeze([
      {
        agent: 'web3-blockchain-expert',
        signals: ['solidity', 'web3', 'blockchain', 'defi', 'smart contract'],
      },
      { agent: 'gamedev-pro', signals: ['unity', 'unreal', 'godot', 'game loop', 'game physics'] },
      {
        agent: 'medical-research-triage',
        signals: ['clinical', 'symptom', 'drug interaction', 'medical literature'],
      },
      {
        agent: 'legacy-modernizer',
        signals: ['legacy code', 'strangler fig', 'modernization', 'monolith upgrade'],
      },
      {
        agent: 'app-generator-agent',
        signals: ['scaffold app', 'generate app', 'bootstrap app', 'prototype app'],
      },
      {
        agent: 'context-manager',
        signals: ['context window', 'session memory', 'context management'],
      },
      {
        agent: 'scientific-research-expert',
        signals: ['genomics', 'simulation', 'academic', 'research workflow'],
      },
    ]),
  }),
});
...
function isHierarchicalRoutingEnabled() {
  return (
    String(process.env.HIERARCHICAL_ROUTING || 'off')
      .trim()
      .toLowerCase() === 'on'
  );
}
