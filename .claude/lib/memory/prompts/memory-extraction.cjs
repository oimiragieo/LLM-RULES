...
function buildTemplate({ summary, recentMessages, user, feedback }) {
  const lines = [
  ...
  if (summary) {
    lines.push('## Session History Summary', summary, '');
  }
...
function getMemoryExtractionPrompt(recentMessages, user = 'default', feedback = '', summary = '') {
  const safeRecent = recentMessages ? String(recentMessages) : '';
  const safeUser = user ? String(user) : 'default';
  const safeFeedback = feedback ? String(feedback) : '';
  const safeSummary = summary ? String(summary) : '';

  return {
    system: 'You extract long-term memories from session context and return strict JSON.',
    user: buildTemplate({
      summary: safeSummary,
...
module.exports = { getMemoryExtractionPrompt };
