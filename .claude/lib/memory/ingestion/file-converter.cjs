...
const TEXT_EXTS = new Set([
    '.txt', '.md', '.csv', '.json', '.yaml', '.yml', '.js', '.cjs', '.mjs',
    '.ts', '.tsx', '.jsx', '.html', '.css', '.sql', '.sh', '.py', '.java',
    '.c', '.cpp', '.h', '.hpp', '.go', '.rs', '.rb', '.php', '.xml'
]);
...
function convertFile(filePath) {
    try {
        ...
        if (stats.size > 15 * 1024 * 1024) { // 15MB limit for text files
        ...
        return { ok: false, error: 'Unsupported file type or binary format', content: null, mimeType: null };
