...
function convertFileInWorker(filePath) {
    return new Promise((resolve, _reject) => {
        ...
        worker.on('error', (err) => resolve({ ok: false, error: err.message, content: null, mimeType: null }));
        worker.on('exit', (code) => {
            if (code !== 0) {
                resolve({ ok: false, error: `Worker stopped with exit code ${code}`, content: null, mimeType: null });
            }
        });
    });
}
 ...
 */
async function writeToFileMemory({ sourcePath, mimeType, content, summary, sizeBytes, isImage, importance = 0.5 }) {
    const db = getDb();

    // Hash for dedup
    const hash = crypto.createHash('sha256').update(content || summary).digest('hex');
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
            id, sourcePath, mimeType, hash, sizeBytes,
            importance, summary, '[]', isImage ? null : content,
            0, ingestedAt
        );
