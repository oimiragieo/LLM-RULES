...
function scoreDimension(name, planContent) {
  ...
  switch (name) {
    case 'requirement-coverage':
      return content.includes('requirement') ||
    ...
    case 'dependency-validity':
      return content.includes('depend') ||
    ...
    case 'risk-assessment':
      return content.includes('risk') ||
        content.includes('mitigation') ||
        content.includes('rollback')
        ? 8
        : 4;
    case 'testability':
      return content.includes('test') || content.includes('verify') || content.includes('assert')
        ? 8
        : 4;
    case 'estimation-quality':
      return content.includes('complexity') ||
...
function verifyPlan(planContent) {
  ...
  return {
    ...
    errors: pass
      ...
      : dimensions
          .filter(d => d.score < 5)
          .map(d => `${d.name}: score ${d.score}/10 (below threshold)`),
  };
}
