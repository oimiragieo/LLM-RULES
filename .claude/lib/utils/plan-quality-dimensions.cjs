...
function assess(name, plan) {
  ...
  switch (name) {
    case 'requirement-coverage': {
      ...
      return { name, score: reqs.length > 0 && tasks.length > 0 ? 1 : reqs.length > 0 ? 0.3 : 0, issues };
    }
    case 'task-completeness': {
      ...
      if (missingAgent.length > 0) issues.push(`${missingAgent.length} task(s) missing agent assignment`);
      const total = tasks.length > 0 ? 1 - (missingDesc.length + missingAgent.length) / (tasks.length * 2) : 0;
      return { name, score: Math.max(0, Math.min(1, total)), issues };
    }
    case 'dependency-validity': {
      const tasks = Array.isArray(plan.tasks) ? plan.tasks : [];
      if (tasks.length === 0) return { name, score: 0.5, issues: ['No tasks to validate dependencies'] };
