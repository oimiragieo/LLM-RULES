...
class OrchestratorService {
    ...
    async processTask(task, context = {}) {
        ...
        if (agentType === 'architect' && result.tasks && Array.isArray(result.tasks)) {
            console.log(`[Orchestrator] Architect returned ${result.tasks.length} sub-tasks. Executing...`);
    ...
    _routeTask(task) {
        ...
        if (t.includes('plan') || t.includes('design') || t.includes('architecture') || t.includes('break down')) {
            return 'architect';
        }
