...
<objective>
Capture an idea, task, or issue that surfaces during an Agent-Studio session as a structured todo for later work.
...
```bash
mkdir -p .claude/todos/pending .claude/todos/done
```
</step>

<step name="check_existing_areas">
```bash
ls .claude/todos/pending/*.md 2>/dev/null | xargs -I {} grep "^area:" {} 2>/dev/null | cut -d' ' -f2 | sort -u
```
...
```bash
grep -l -i "[key words from title]" .claude/todos/pending/*.md 2>/dev/null
```
...
Write to `.claude/todos/pending/${date_prefix}-${slug}.md`:
...
If `.claude/state/current-task.json` exists:
1. Read current state
2. Add todo reference to state
3. Update state file
</step>
...
<output>
- `.claude/todos/pending/[date]-[slug].md`
- Updated `.claude/state/current-task.json` (if exists)
</output>
