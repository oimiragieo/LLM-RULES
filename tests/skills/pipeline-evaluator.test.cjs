...
test('skill contains scoring dimensions', () => {
  const content = fs.readFileSync(path.join(PROJECT, '.claude/skills/pipeline-evaluator/SKILL.md'), 'utf8');
...
test('schema has required evaluation fields', () => {
  const schema = JSON.parse(fs.readFileSync(path.join(PROJECT, '.claude/schemas/pipeline-evaluation.schema.json'), 'utf8'));
