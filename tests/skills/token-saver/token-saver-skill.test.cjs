...
const SKILL_MD = path.join(PROJECT_ROOT, '.claude/skills/token-saver-context-compression/SKILL.md');
...
test('SKILL.md has Activation section', () => {
  ...
  assert.ok(content.includes('## Activation'), 'Expected "## Activation" section in SKILL.md');
});
...
test('SKILL.md references 80K token threshold', () => {
  const content = fs.readFileSync(SKILL_MD, 'utf8');
  assert.ok(content.includes('80K'), 'Expected 80K token threshold reference in SKILL.md');
});
