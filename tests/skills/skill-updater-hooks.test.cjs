...
test('pre-execute validates skill and trigger', () => {
  const ok = spawnSync(process.execPath, [preHook, JSON.stringify({ skill: 'tdd', trigger: 'reflection' })], {
    encoding: 'utf8',
  });
  ...
  const bad = spawnSync(process.execPath, [preHook, JSON.stringify({ skill: 'tdd', trigger: 'bad-trigger' })], {
    encoding: 'utf8',
  });
...
test('post-execute emits compact summary', () => {
  const proc = spawnSync(
    process.execPath,
    [postHook, JSON.stringify({ ok: true, mode: 'plan', trigger: 'evolve', target: { skillName: 'tdd' } })],
    { encoding: 'utf8' }
  );
