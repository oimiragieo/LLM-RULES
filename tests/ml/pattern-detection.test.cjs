...
function generateSession(type) {
    if (type === 'quick-fix') {
        ...
        return {
            history: [{ agent: 'architect' }, { agent: 'planner' }, { agent: 'developer' }, { agent: 'security' }],
...
describe('SPEC-023: Pattern Detection Engine', () => {
    ...
    describe('FeatureEngineer', () => {
        it('normalizes metrics correctly', () => {
            ...
            assert.ok(features.totalDuration > 0 && features.totalDuration < 1, 'Duration should be normalized');
            assert.ok(features.totalTokens > 0 && features.totalTokens < 1, 'Tokens should be normalized');
            assert.ok(features.sequenceLength >= 0 && features.sequenceLength <= 1, 'sequenceLength should be normalized 0-1');
    ...
    describe('KMeans Clustering', () => {
        it('groups identical points together', () => {
            ...
            const data = [
                { x: 0, y: 0 }, { x: 0.1, y: 0.1 }, // Cluster A
                { x: 10, y: 10 }, { x: 9.9, y: 9.9 } // Cluster B
            ];
            ...
            assert.notEqual(p1.clusterId, p2.clusterId, 'Should assign different clusters to distant points');
    ...
    describe('PatternDetector (Integration)', () => {
        it('identifies distinct workflow patterns', () => {
            ...
            assert.notEqual(analysisQuick.patternId, analysisArch.patternId, 'Should distinguish workflow types');
        });
