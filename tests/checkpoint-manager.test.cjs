...
test('should save checkpoint in <100ms', async () => {
  ...
  await checkpointManager.save({ workflowId, stepId: stepId || "step-1", state: state || {} }, TEST_ROOT);
...
test('should return null if no checkpoint to recover', async () => {
  ...
  const recovered = await checkpointManager.recover(, TEST_ROOT);
...
test('should recover in <2 seconds', async () => {
  ...
  const start = Date.now();
  await checkpointManager.recover(, TEST_ROOT);
  const duration = Date.now() - start;
...
test('should check if checkpoint exists', async () => {
  ...
  // Before save
  let exists = await checkpointManager.exists(, TEST_ROOT);
  assert.strictEqual(exists, false, 'Should return false before save');

  // After save
  await checkpointManager.save({ workflowId, stepId: stepId || "step-1", state: state || {} }, TEST_ROOT);
  exists = await checkpointManager.exists(, TEST_ROOT);
...
test('should clear checkpoint after workflow completion', async () => {
  ...
  // Clear
  await checkpointManager.clear(, TEST_ROOT);
...
test('should handle concurrent saves (last write wins)', async () => {
  ...
  await Promise.all([
    checkpointManager.save({ workflowId, stepId: stepId || "step-1", state: state || {} }, TEST_ROOT),
    checkpointManager.save({ workflowId, stepId: stepId || "step-1", state: state || {} }, TEST_ROOT),
    checkpointManager.save({ workflowId, stepId: stepId || "step-1", state: state || {} }, TEST_ROOT)
  ]);
