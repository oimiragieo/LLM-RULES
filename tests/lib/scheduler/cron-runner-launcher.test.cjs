...
const LAUNCHER_PATH = path.join(PROJECT_ROOT, '.claude', 'tools', 'cli', 'cron-session-launcher.cjs');
...
test('cron-session-launcher', async (t) => {
    ...
    if (fs.existsSync(PID_FILE)) {
        try { fs.unlinkSync(PID_FILE); } catch (_e) { /* best-effort cleanup */ }
    }
    ...
    await t.test('fails when ANTHROPIC_API_KEY is missing', () => {
        ...
        assert.match(res.stderr, /ANTHROPIC_API_KEY environment variable is required/, 'Should print fatal error');
    });
    ...
    await t.test('PID lock - O_EXCL race prevention (M4 test)', () => {
        ...
        assert.match(res.stdout, /Found running cron-runner instance/, 'Should log that instance is already running');

        // The PID file should remain untouched
        const currentPid = fs.readFileSync(PID_FILE, 'utf8');
        assert.strictEqual(currentPid, String(process.pid), 'Should not modify the lockfile owned by alive process');
    ...
    await t.test('PID lock - recovers from stale lock', () => {
        ...
        assert.strictEqual(newPid, String(process.pid), 'Should have overwritten stale lock with current process PID');
