...
test('Code Review Routing Optimization', async t => {
  const mockAgents = [
    ...
    {
      name: 'architect',
      description: 'Designs system architecture and topology',
      priority: 'high',
    },
  ...
  await t.test(
    'should prioritize code-reviewer over architect for a pure code review prompt',
    () => {
      const prompt =
        'Please review my recent changes in the login controller for any potential bugs.';
  ...
  await t.test('should still include architect if architecture signals are present', () => {
    const prompt =
      'Review the code but also check if the new database schema fits the system architecture.';
    const classification = { intent: 'code_review', defaultAgent: 'code-reviewer' };

    const { candidates } = scoreAgents(prompt, mockAgents, classification);

    assert.strictEqual(
      candidates.some(c => c.agent.name === 'code-reviewer'),
      true
    );
    assert.strictEqual(
      candidates.some(c => c.agent.name === 'architect'),
      true
    );
  });
});
