...
test('External Integration Routing', async (t) => {
  ...
  await t.test('should prefer artifact-integrator for research on a github repo (disambiguation)', () => {
    const prompt = 'Please research this github repo: hmohamed01/powershell-expert and turn it into a skill.';
    const classification = classifyIntent(prompt);
