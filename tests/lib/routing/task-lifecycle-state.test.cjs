...
describe('Task Lifecycle State Machine', () => {
  ...
  test('isValidTransition should validate standard lifecycle', () => {
    ...
    assert.strictEqual(isValidTransition('in_progress', 'completed'), true, 'in_progress -> completed');
  ...
  test('isValidTransition should block invalid transitions', () => {
    assert.strictEqual(isValidTransition('pending', 'completed'), false, 'pending -> completed (must be in_progress first)');
    assert.strictEqual(isValidTransition('completed', 'in_progress'), false, 'completed -> in_progress');
  ...
  test('getTransitionError should provide helpful messages', () => {
    ...
    assert.match(error, /must go through in_progress first/, 'Error message for pending->completed');
