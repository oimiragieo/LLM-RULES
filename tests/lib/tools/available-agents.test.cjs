...
describe('AvailableAgents Tool', () => {
  ...
  describe('Registry Consistency', () => {
    ...
    it('warns on registry drift by default', () => {
      ...
      const q = new AvailableAgentsQuery({
        registryPath: CONSISTENCY_REGISTRY,
        agentsDir: path.join(CONSISTENCY_DIR, '.claude', 'agents'),
        enableConsistencyCheck: true,
        consistencyCheckIntervalMs: 0,
      });
      const result = q.query({ minSuccessRate: 0 });

      console.warn = originalWarn;

      assert.strictEqual(result.success, true);
      assert.ok(warnings.length > 0, 'Should emit consistency warning');
      assert.ok(
        warnings.some(msg => msg.includes('Agent registry drift detected')),
        'Should warn about registry drift'
      );
    });
    ...
    it('blocks when REGISTRY_CONSISTENCY_GATE=block', () => {
      ...
      assert.strictEqual(result.success, false);
      assert.ok(
        result.error.includes('Agent registry drift detected'),
        'Should surface drift error'
      );
    });
  });
});
