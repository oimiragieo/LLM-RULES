...
test('SEC-TMPL-004: sanitizeSubstitutionValue function tests', async t => {
  ...
  await t.test('should export sanitizeSubstitutionValue function', () => {
    ...
    assert.strictEqual(
      typeof sanitizeSubstitutionValue,
      'function',
  ...
  await t.test('should sanitize nested {{nested{{deep}}}}', () => {
    ...
    assert.ok(!result.includes('{{'), 'Should remove all {{ (no spaces)');
    assert.ok(!result.includes('}}'), 'Should remove all }} (no spaces)');

    // Should contain sanitized version: "{ {nested{ {deep} }} }"
    assert.ok(result.includes('{ {'), 'Should contain sanitized { {');
    assert.ok(result.includes('} }'), 'Should contain sanitized } }');
  });
  ...
  await t.test('should leave normal values unchanged', () => {
    ...
    assert.strictEqual(result, normalValue, 'Normal values should remain unchanged');
  });
  ...
  await t.test('should preserve single braces', () => {
    ...
    assert.strictEqual(result, singleBraces, 'Single braces should not be affected');
  });
});
