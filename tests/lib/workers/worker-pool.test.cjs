 ...
 */
function makePool(processFn, { concurrency = 3, failSafeIntervalMs = 50, heartbeatIntervalMs = 500 } = {}) {
  const dbPath = makeTempDbPath();
  const db = getDb(dbPath);
  runMigrations(db);
  const budget = new BudgetEnforcementService({ maxTokensPerMinute: 400000, maxConcurrentWorkers: concurrency });
  const pool = new WorkerPool({ db, budget, concurrency, processFn, failSafeIntervalMs, heartbeatIntervalMs });
  ...
  describe('message processing', () => {
    ...
    it('failed processFn results in pending or dead_letter status', (_, done) => {
      ...
      pool.on('worker-error', ({ id }) => {
        const row = db.prepare('SELECT status, attempt_count FROM message_queue WHERE id = ?').get(id);
    ...
    it('message is dead_lettered after 3 failed attempts', (_, done) => {
    ...
    it('multiple messages are processed concurrently', (_, done) => {
