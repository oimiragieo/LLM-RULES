...
const CALCULATOR = path.join(
  process.cwd(),
  '.claude',
  'tools',
  'cli',
  'semver-bump-calculator.cjs'
);
...
test('semver-bump-calculator correctly handles diff command', () => {
  const result = spawnSync('node', [CALCULATOR, 'diff', '--from', '1.0.0', '--to', '1.1.0'], {
    encoding: 'utf8',
  });
...
test('semver-bump-calculator computes frontmatter bump correctly', () => {
  ...
  const result = spawnSync(
    'node',
    [CALCULATOR, '--old', oldFile, '--new', newFile, '--type', 'agent'],
    { encoding: 'utf8' }
  );
