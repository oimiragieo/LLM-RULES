...
const budgetTrackerPath = require.resolve(
  path.join(PROJECT_ROOT, '.claude', 'lib', 'utils', 'token-budget-tracker.cjs')
);
...
test('Token Guard allows tools when tokens are BELOW the safe limit (< 180k)', () => {
  ...
  if (result && result.action === 'block') {
    assert.ok(
      !result.message.includes('[SYSTEM URGENT] Context pressure'),
      'Should not block with urgent token message'
    );
  } else {
    assert.ok(true);
  }
...
test('Token Guard BLOCKS tools with SYSTEM URGENT when tokens exceed 180k and EXTRA_USAGE_ENABLED is false', () => {
  ...
  assert.ok(
    result.message.includes('[SYSTEM URGENT] Context pressure > 180k tokens'),
    'Must enforce 180k constraint'
  );
});
...
test('Token Guard PERMITS critical lifecycle tools (TaskOutput, TaskList, TaskUpdate) even when tokens exceed 180k', () => {
  ...
  if (result && result.action === 'block') {
    assert.ok(
      !result.message.includes('[SYSTEM URGENT] Context pressure'),
      'Must NOT block lifecycle tools'
    );
  } else {
    assert.ok(true);
  }
...
test('Token Guard limits Opus context to 180k when EXTRA_USAGE_ENABLED is not set (Anthropic API Default)', () => {
  ...
  assert.ok(
    result.message.includes('[SYSTEM URGENT] Context pressure > 180k tokens'),
    'Must enforce 180k constraint by default'
  );
});
