...
const DETECTOR_SCRIPT = path.join(PROJECT_ROOT, '.claude', 'hooks', 'session', 'stale-task-detector.cjs');
const TASKUPDATE_STATE_FILE = path.join(PROJECT_ROOT, '.claude', 'context', 'runtime', 'taskupdate-first-state.json');
const GAP_LOG_FILE = path.join(PROJECT_ROOT, '.claude', 'context', 'runtime', 'session-gap-log.jsonl');
const COOLDOWN_FILE = path.join(PROJECT_ROOT, '.claude', 'context', 'runtime', 'stale-task-emission-cooldown.json');

function backupFile(filePath) { return fs.existsSync(filePath) ? fs.readFileSync(filePath, 'utf8') : null; }
function restoreFile(filePath, backup) { if (backup !== null) { fs.writeFileSync(filePath, backup, 'utf8'); } else { fs.rmSync(filePath, { force: true }); } }

function runDetector(env) {
  return new Promise(resolve => {
    const child = spawn(process.execPath, [DETECTOR_SCRIPT], {
      stdio: ['pipe','pipe','pipe'],
      env: { ...process.env, ...(env||{}), STALE_TASK_AUTO_QUEUE: 'off', STALE_TASK_THRESHOLD_MS: '1', STALE_TASK_EMISSION_COOLDOWN_MS: String(60*60*1000) }
    });
    let out = ''; let err = '';
...
describe('stale-task-detector cooldown deduplication', { concurrency: 1 }, () => {
  let stateBackup = null; let cooldownBackup = null; let gapLogBackup = null; let _gapLogBaseLen = 0;
  ...
  before(() => {
    ...
    _gapLogBaseLen = gapLogBackup.length;
    fs.mkdirSync(path.dirname(TASKUPDATE_STATE_FILE), { recursive: true });
    fs.writeFileSync(TASKUPDATE_STATE_FILE, JSON.stringify({ sessions: { [TEST_SESSION]: { inProgress: true, taskId: TEST_TASK, updatedAt: Date.now() - 30*60*1000 } } }, null, 2));
  ...
  test('first run emits stale warning and writes cooldown file', async () => {
    ...
    assert.ok(Object.prototype.hasOwnProperty.call(cd, TEST_TASK), 'cooldown file must contain task id');
  });

  test('second run within cooldown window suppresses emission', async () => {
    const before = fs.existsSync(GAP_LOG_FILE) ? fs.readFileSync(GAP_LOG_FILE,'utf8') : '';
    const { stderr } = await runDetector();
    assert.ok(!stderr.includes(TEST_TASK), 'second run must NOT re-emit STALE-TASK for '+TEST_TASK);
