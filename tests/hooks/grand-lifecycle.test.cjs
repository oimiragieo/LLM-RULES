...
const PRE_TASK_HOOK = path.join(PROJECT_ROOT, '.claude', 'hooks', 'routing', 'pre-task-unified.cjs');
const PRE_TOOL_HOOK = path.join(PROJECT_ROOT, '.claude', 'hooks', 'routing', 'pre-tool-unified.cjs');
const POST_TASK_HOOK = path.join(PROJECT_ROOT, '.claude', 'hooks', 'routing', 'post-task-unified.cjs');
const POST_COMPLETION_HOOK = path.join(PROJECT_ROOT, '.claude', 'hooks', 'workflow', 'post-completion-chain.cjs');
...
async function testLifecycle() {
  ...
  if (!fs.existsSync(path.dirname(dummyPlan))) fs.mkdirSync(path.dirname(dummyPlan), { recursive: true });
  ...
  if (res3.code !== 0) throw new Error(`TaskUpdate(in_progress) blocked: ${res3.stderr || res3.stdout}`);
