...
test('Sample Error Capture Scenario', async t => {
  await t.test('simulate tool error and verify sensitive data masking', async () => {
    // Load the sanitizer
    const sanitizer = require(path.join(PROJECT_ROOT, '.claude/lib/utils/error-sanitizer.cjs'));
    ...
    const errorContext = {
      ...
      connectionString: 'mongodb://admin:secretPassword123@localhost:27017/mydb',
    ...
    assert.ok(!sanitized.jwtToken.includes('eyJ'), 'jwtToken should not contain original value');
  ...
  await t.test('error writer can create error entry with sanitized context', async () => {
    const writer = require(path.join(PROJECT_ROOT, '.claude/lib/error-writer.cjs'));
    ...
    assert.ok(
      result === undefined || result === null || result === true,
      'writeError should complete without throwing'
    );
  });
});
