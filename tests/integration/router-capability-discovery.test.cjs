...
describe('Router Capability Discovery Integration', () => {
  ...
  describe('10. Integration with Health Tracker', () => {
    it('should correctly parse health status from registry', () => {
      ...
      // Query including all by lowering thresholds
      const result = queryEngine.query({
        capability: 'implementation',
        excludeFailed: false,
        minSuccessRate: 0.0
      ...
      assert.strictEqual(result.success, true);
      assert.strictEqual(result.count, 3, 'Should return all 3 agents');

      // Verify health status is accessible
      const healthy = result.agents.find(a => a.id === 'agent-1');
      const degraded = result.agents.find(a => a.id === 'agent-2');
      const unavailable = result.agents.find(a => a.id === 'agent-3');

      assert.strictEqual(healthy.health.status, 'healthy');
      assert.strictEqual(degraded.health.status, 'degraded');
      assert.strictEqual(unavailable.health.status, 'unavailable');
    });
  ...
  describe('11. Helper Methods', () => {
    it('getAgent should return specific agent by ID', () => {
      ...
      assert.ok(agent, 'Should return agent');
    ...
    it('getBestAgent should return highest rated agent', () => {
      ...
      assert.ok(best, 'Should return best agent');
  ...
  describe('12. Domain-Based Fallback', () => {
    it('should find agents by domain when capability not found', () => {
      ...
      // Query by domain when capability not available
      const result = queryEngine.query({ domain: 'code' });

      assert.strictEqual(result.success, true);
      assert.strictEqual(result.count, 2, 'Should find both code domain agents');
    });
  });
});
