...
async function testAccessTracking() {
  ...
  if (entry.lastAccessed !== null)
    throw new Error(`Expected lastAccessed null, got ${entry.lastAccessed}`);
  ...
  console.log(
    `[TEST] Access update verified (accessCount: ${entry.accessCount}, lastAccessed: ${entry.lastAccessed})`
  );
